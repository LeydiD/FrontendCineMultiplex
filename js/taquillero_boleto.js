...
async function buscarFuncionesPorFecha(fecha) {
  try {
    const response = await fetch(
      `http://localhost:8080/funciones/fecha/${fecha}`,
      {
        method: "GET",
        headers: {
...
function mostrarFuncionesEnSelector(funciones) {
  ...
  funciones.forEach((funcion) => {
    ...
    option.textContent = `Película: ${
      funcion.pelicula.titulo || "Sin título"
    }, Hora: ${funcion.horario || "Sin hora"}`;
...
movieShowSelect.addEventListener("change", async (event) => {
  ...
  if (selectedFunctionId) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          "No se pudo obtener la información de la función seleccionada."
        );
      }
...
async function mostrarAsientos(salaId, funcionId) {
  try {
    const response = await fetch(
      `http://localhost:8080/asientos/sala/${salaId}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    const responseOcupados = await fetch(
      `http://localhost:8080/boletos/funcion/${funcionId}/asientos`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    theaterSeatingLayout.innerHTML =
    ...
    Object.keys(asientosPorFila).forEach((fila) => {
      ...
      asientosPorFila[fila].forEach((asiento) => {
        ...
        const isOccupied = asientosOcupados.some(
          (ocupado) =>
            ocupado.letra === asiento.letra &&
            ocupado.numeroAsiento === asiento.numeroAsiento
        );
        if (isOccupied) {
          ...
          asientoButton.addEventListener("click", () =>
            seleccionAsientos(
              asientoButton.dataset.seatId,
              asientoButton,
              asiento
            )
          );
...
async function cargarCategorias() {
  try {
    const response = await fetch(
      "http://localhost:8080/api/categorias-boletos"
    );
    if (!response.ok) {
    ...
    categorySelect.addEventListener("change", function () {
      ...
      if (selectedCategory) {
        ...
        totalBoletosConDescuento =
          selectedCategory.precio * selectedSeats.length;
        document.getElementById(
          "total-price"
        ).textContent = `Total boletos: $${totalBoletosConDescuento.toLocaleString()}`;
        cargarDescuento(
          selectedCategory.id,
          totalBoletosConDescuento,
          nameCategory
...
async function cargarDescuento(categoriaId, precioTotal, nameCategory) {
  try {
    const response = await fetch(
      "http://localhost:8080/api/categoria-boleto-promocion"
    );
    if (!response.ok) {
      throw new Error("No se pudieron obtener los descuentos");
    }
    const promociones = await response.json();
    const descuentoAplicable = promociones.find(
      (promocion) => promocion.categoriaBoleto.id === categoriaId
    );
    ...
    if (descuentoAplicable) {
      ...
      ).textContent = `Válido para aplicar descuento de ${descuento}% en boletos ${nameCategory}`;
      document.getElementById(
        "discounted-price"
      ).textContent = `Precio con descuento aplicado: $${precioConDescuento.toLocaleString(
        "es-CO"
      )}`;
  ...
  try {
    ...
    snacks.forEach((snack) => {
      const snackPromo = promociones.find(
        (promo) => promo.snack.id === snack.id
      );
      const descuento = snackPromo ? snackPromo.descuento : 0;
      const precioConDescuento = snackPromo
        ? snack.precio * (1 - descuento / 100)
        : snack.precio;
      ...
      snackCard.innerHTML = `
                ...
                ${
                  descuento > 0
                    ? `<p>Descuento: ${descuento}%</p><p>Precio con descuento: $${precioConDescuento.toLocaleString()}</p>`
                    : ""
                }
                ...
                <input type="number" id="quantity-${
                  snack.id
                }" class="snack-quantity" value="0" min="0" max="${
        snack.cantidadDisponible
      }" data-snack-id="${snack.id}" data-snack-price="${precioConDescuento}" />
          </div>  `;
...
function mostrarsnacks(snacksFiltradas) {
  ...
  snacksFiltradas.forEach((snack) => {
    ...
    snackCard.innerHTML = `
            ...
            <!-- Div para agrupar el label y el input de cantidad -->
            <div class="quantity-container">
              <label for="quantity-${snack.id}">Cantidad:</label>
              <input type="number" id="quantity-${
                snack.id
      ...
      snack.cantidadDisponible
    }" data-snack-id="${snack.id}" data-snack-price="${snack.precio}" />
            </div>
        `;
    snackContainer.appendChild(snackCard);
  });
...
document.getElementById("calculate-total").addEventListener("click", () => {
  ...
  document.getElementById(
    "combined-total"
  ).textContent = `Total a pagar: ${formattedTotal}`;
});
...
async function cargarPromocionesSnacks() {
  try {
    ...
    if (!response.ok)
      throw new Error("Error al cargar las promociones de snacks");
    const promociones = await response.json();
    return promociones;
  } catch (error) {
...
document.addEventListener("input", (event) => {
  if (event.target.classList.contains("snack-quantity")) {
    ...
    document.querySelectorAll(".snack-quantity").forEach((input) => {
      ...
      snackTotal += subtotal;
      if (cantidad > 0) {
        const existingSnack = selectedSnacks.find(
...
import Pedido from "./taquillero_pedido.js";
import { clientId } from "./taquillero_cliente.js";
import { clienteNombre } from "./taquillero_cliente.js";
...
reserveButton.addEventListener("click", async () => {
  if (selectedSeats.length > 0 && selectedShow) {
    if (!clientId) {
      alert(
        "Por favor, busca o registra un cliente antes de realizar la reserva."
      );
      return;
    }
    ...
    try {
      ...
      document.getElementById("order-total").textContent = `$${(
        totalBoletosConDescuento + snackTotal
...
confirmOrderButton.addEventListener("click", async () => {
  ...
  try {
    ...
    alert(
      `Pedido realizado con éxito. Boletos generados: ${boletosCreados.length}`
    );
    modal.style.display = "none";
...
async function actualizarCantidadSnacks() {
  try {
    for (const snack of selectedSnacks) {
      ...
      const response = await fetch(
        `http://localhost:8080/snacks/${encodeURIComponent(
          snackId
        )}/restar/${cantidad}`,
        {
          method: "PATCH",
